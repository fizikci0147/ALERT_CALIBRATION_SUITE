 ...
 */
package org.clas.modules.wc.calt2d;

import org.clas.modules.wc.analysis.Coordinate;
import org.clas.modules.wc.analysis.FitPanel;
import org.clas.modules.wc.t2d.TableLoader;
import org.clas.modules.wc.t2d.TimeToDistanceEstimator;
import org.clas.modules.wc.viewer.AnalysisMonitor;
import org.clas.modules.wc.viewer.T2DViewer;
...
import static org.clas.modules.wc.viewer.T2DViewer.ccdb;
...
public class T2DCalib extends AnalysisMonitor{
    ...
    public void createHistos() {
        ...
        for (int i = 0; i < nBoard; i++) {
            ...
            timeResi.put(new Coordinate(i), new H1F("time residual for Board" + (i+1), 100, -0.5, 0.5));
            timeResiFromFile.put(new Coordinate(i), new H1F("time residual for Board " + (i+1), 100, -0.5, 0.5));
            timeResiNew.put(new Coordinate(i), new H1F("time residual for Board " + (i+1), 100, -0.5, 0.5));
            fitResi.put(new Coordinate(i), new H1F("fit residual for Board " + (i+1), 100, -0.5, 0.5));
    ...
    public void plotHistos() {
        ...
        this.getAnalysisCanvas().getCanvas(Names[3]).divide(this.nBoard, 2);
      ...
      //  TvstrkdocasFit.put(new Coordinate(i),
       //        new FitFunction(i, (Map<Coordinate, GraphErrors>) TvstrkdocasProf));
        ...
        for(int it = 0; it<maxIter; it++) {
            ...
            min = migrad.minimize();
            System.err.println("****************************************************");
            System.err.println("*   FIT RESULTS  FOR CHANNEL  "+(i+1)+" at iteration "+(it+1)+"  *");
            System.err.println("****************************************************");  
    ...
    public void reProcess() {
        ...
        this.getAnalysisCanvas().getCanvas("Time Residuals").divide(nBoard, 3);
        //
        for(int i = 0; i<this.nBoard; i++) {
